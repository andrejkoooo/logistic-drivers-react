...
const Drivers = () => {
  ...
  const { drivers, loading, error, daysOfWeek } = useSelector(
    (state: RootState) => state.drivers
  );
  const [filteredDrivers, setFilteredDrivers] = useState([] as ITableDriver[]);
  ...
  const onSearchChange = (searchText: string) => {
    const filteredDrivers = drivers.filter((driver: ITableDriver) => {
      const drivers = driver.name
        .toLowerCase()
        .includes(searchText.toLowerCase());
      const vehicles = driver.vehicleRegistration
        .toLowerCase()
        .includes(searchText.toLowerCase());
